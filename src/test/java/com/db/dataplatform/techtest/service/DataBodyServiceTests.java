...
public class DataBodyServiceTests {
    ...
    @Test
    public void shouldGetDataByBlockTypeAsExpected() throws Exception {
        String blockType = "BLOCKTYPEA";
        dataBodyService.getDataByBlockType(BlockTypeEnum.valueOf(blockType));

        verify(dataStoreRepositoryMock, times(1)).findByBlockType(eq(BlockTypeEnum.BLOCKTYPEA));
    }

    @Test
    public void shouldGetDataByBlockNameAsExpected() throws Exception {
        String blockName = "Test";
        dataBodyService.getDataByBlockName(blockName);

        verify(dataStoreRepositoryMock, times(1)).findByBlockName(eq(blockName));
    }
